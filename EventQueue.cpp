...
Attack::Attack(int p_time, Computer *p_target) : Event(p_time), target(p_target) {}
...
void Attack::execute(int time_sim) {
    if (!canAttack()) return;

    bool success = rand() % 100 < Globals::perc_success,
            detect = rand() % 100 < Globals::perc_detect;
    ...
    if (success) {
        if (!target->infected) {
            ...
            Globals::events.addEvent(*new ComputerAttack(time_sim + 10, target, &target->randomOther()));
    ...
    if (detect) notify(time_sim + 1);

    relaunch(time + 10);
}

ComputerAttack::ComputerAttack(int p_time, Computer *p_source, Computer *p_target) : Attack(p_time, p_target),
                                                                                     source(p_source) {}
...
void ComputerAttack::print(bool success, bool detect) {
    cout << "Attack(" << time << ", " << source->id << ", " << target->id << ") ";

    if (success) cout << "SUCCESS";
    else cout << "FAIL   ";

    if (detect) cout << "DETECT";
    else cout << "      ";
...
void ComputerAttack::relaunch(int newtime) {
    Globals::events.addEvent(*new ComputerAttack(newtime, source, &source->randomOther()));
}

AttackerAttack::AttackerAttack(int p_time, Computer *p_target) : Attack(p_time, p_target) {}

void AttackerAttack::print(bool success, bool detect) {
    cout << "Attack(" << time << ", ATKR, " << target->id << ") ";
...
void AttackerAttack::relaunch(int newtime) {
    Globals::events.addEvent(*new AttackerAttack(newtime, &Globals::computers[rand() % Globals::num_computers]));
}

Notify::Notify(int p_time, Computer *p_source, Computer *p_target) : Event(p_time) {
    source = p_source;
    target = p_target;
}

Notify::Notify(int p_time, Computer *p_target) : Notify(p_time, nullptr, p_target) {}

void Notify::execute(int time_sim) {
    cout << "Notify(" << time_sim << ", ";
    ...
    if (!Globals::fixInProgress) {
        Globals::events.addEvent(*new Fix(time_sim + 101, Globals::fixes.front()));
        Globals::fixes.pop();
    }
}
...
void Fix::execute(int sim_time) {
    if (target->infected) {
        ...
        if (Globals::num_infected == 0)
            Globals::sim_state = Globals::SysadminWon;
    }

    if (Globals::fixes.size()) {
        Globals::events.addEvent(*new Fix(sim_time + 100, Globals::fixes.front()));
        Globals::fixes.pop();
    } else Globals::fixInProgress = false;
}
...
void EventQueue::executeEvent(int time_sim) {
    assert(size_array > 0);

    // execute top node of the array, delete top node, subtract 1 size from heap, remake the heap so it maintains heap structure
    array[1]->execute(time_sim);
    delete array[1];
...
void EventQueue::percolateUp(int index) {
    ...
    while (true) {
        ...
        if (array[index]->time < array[index / 2]->time)
...
void EventQueue::percolateDown(int index) {
    ...
    while (true) {
        ...
        if (child + 1 <= size_array && array[child + 1]->time < array[child]->time)
            ++child;

        if (array[child]->time < array[index]->time)
            std::swap(array[index], array[index / 2]);
